...
class aclient(discord.Client):
    def __init__(self) -> None:
        ...
        self.activity = discord.Activity(type=discord.ActivityType.listening, name="/chat | /hilfe")
    ...
    async def send_message(self, message, user_message):
        ...
        try:
            ...
            if self.is_replying_all == "True":
                ...
                await message.followup.send("> **Hoppla: Irgendetwas ist schiefgelaufen, bitte versuche es später noch einmal!**")
            ...
            if os.path.isfile(prompt_path) and os.path.getsize(prompt_path) > 0:
                with open(prompt_path, "r", encoding="utf-8") as f:
                    ...
                    if (discord_channel_id):
                        logger.info(f"Sende Startprompt mit der Größe: {len(prompt)}")
                        response = ""
                        if self.chat_model == "OFFICIAL":
                ...
                logger.info(f"No {prompt_name}. Überspringe das senden eines Startprompts.")
        except Exception as e:
            logger.exception(f"Fehler beim senden des Startprompts: {e}")
